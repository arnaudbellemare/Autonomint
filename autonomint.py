...
def fetch_instruments():
    response = requests.get(URL_INSTRUMENTS, timeout=REQUEST_TIMEOUT); response.raise_for_status(); return response.json().get("result", [])

@st.cache_data(ttl=10)
@with_retries()
def fetch_ticker(instr_name: str):
    time.sleep(API_DELAY_TICKER); response = requests.get(URL_TICKER, params={"instrument_name": instr_name}, timeout=REQUEST_TIMEOUT); response.raise_for_status(); return response.json().get("result", {})
...
def calculate_realized_volatility(prices: pd.Series, window: int = 30) -> pd.Series:
    log_returns = np.log(prices / prices.shift(1)); return log_returns.rolling(window=window).std() * np.sqrt(365)

@st.cache_data(ttl=900)
def get_volatility_metrics(realized_vol_series: pd.Series, current_iv: float):
    current_rv = realized_vol_series.iloc[-1]
    if pd.isna(current_rv) or current_rv == 0: return None, None

    # --- SIMULATION OF HISTORICAL IV ---
    np.random.seed(42)
    iv_noise = np.random.normal(0.05, 0.2, len(realized_vol_series))
    historical_iv = (realized_vol_series * (1 + iv_noise)).clip(lower=0.1)
    # --- END SIMULATION ---
    
    iv_rv_ratio_series = (historical_iv / realized_vol_series).dropna()
    log_iv_rv_ratio_series = np.log(iv_rv_ratio_series)

    if len(log_iv_rv_ratio_series) < 20: return None, None
    
    mean_log_ratio = log_iv_rv_ratio_series.mean()
    std_log_ratio = log_iv_rv_ratio_series.std()

    # Calculate metrics for the CURRENT market condition
    current_iv_rv_ratio = current_iv / current_rv
    current_log_ratio = np.log(current_iv_rv_ratio)
    z_score = (current_log_ratio - mean_log_ratio) / std_log_ratio if std_log_ratio > 0 else 0

    return z_score, current_iv_rv_ratio
...
def _calculate_dte(expiry_str, current_date_utc):
    try:
        ...
        time_to_expiry = expiry_dt_obj - current_date_utc; return time_to_expiry.days + (time_to_expiry.seconds / (24 * 3600))
...
def create_payoff_diagram(strategy_details, price_range, eth_deposited, eth_price_initial, aave_yield, predicted_price):
    ...
    total_pnl = underlying_pnl + aave_yield; title_text = "Payoff: ETH Holdings + AAVE Yield"
    if strategy_details['name'] == "Sell Premium 🤑":
        short_put = strategy_details.get('put'); short_call = strategy_details.get('call'); option_pnl = 0; option_titles = []
        if short_put is not None:
            put_pnl = short_put['price'] - np.maximum(0, short_put['strike'] - price_range); option_pnl += put_pnl; option_titles.append(f"Short Put @ ${short_put['strike']:.0f}")
        if short_call is not None:
            call_pnl = short_call['price'] - np.maximum(0, price_range - short_call['strike']); option_pnl += call_pnl; option_titles.append(f"Short Call @ ${short_call['strike']:.0f}")
        total_pnl += option_pnl
        if option_titles: title_text += " + " + " & ".join(option_titles)
    fig = go.Figure()
...
with st.sidebar:
    ...
    st.header("📈 Hedging & Yield"); AAVE_APY_PERCENT = st.slider("AAVE Supply APY", 0.0, 10.0, 3.0, format="%.2f%%"); AAVE_APY = AAVE_APY_PERCENT / 100.0
    st.header("🧠 Option Selling Parameters"); TARGET_DTE = st.slider("Target Days to Expiry (DTE)", 7, 90, 30); Z_SCORE_SELL_THRESHOLD = st.slider("Z-Score Sell Threshold", 0.5, 3.0, 1.0, 0.1, help="Sell options if the Log(IV/RV) Z-Score is above this value."); TARGET_DELTA = st.slider("Target Delta", 0.10, 0.45, 0.35, 0.01, help="The target delta for the puts/calls to sell."); MIN_PREMIUM_RATIO = st.slider("Min Premium-to-Spot Ratio (%)", 0.1, 5.0, 1.0, 0.1, help="The minimum premium required (as % of spot price) to sell an option.") / 100.0
...
if not current_iv: st.warning("Could not fetch a reference Implied Volatility for analysis.");

# --- NEW: Calculate AND Display both Z-Score and Raw Ratio ---
z_score, iv_rv_ratio = (None, None)
if current_iv > 0 and not realized_vol_series.empty and not pd.isna(realized_vol_series.iloc[-1]):
    z_score, iv_rv_ratio = get_volatility_metrics(realized_vol_series, current_iv)

st.header("Volatility & Strategy Signal")
v_col1, v_col2, v_col3, v_col4 = st.columns(4)
v_col1.metric("Live ETH Price", f"${live_eth_price:,.2f}")
v_col2.metric("30D Realized Volatility (RV)", f"{realized_vol_series.iloc[-1]:.2%}" if not realized_vol_series.empty else "N/A", help="Historical volatility over the last 30 days.")
v_col3.metric("ATM Implied Volatility (IV)", f"{current_iv:.2%}" if current_iv > 0 else "N/A", help="Market's expectation of future volatility.")
v_col4.metric("IV/RV Ratio", f"{iv_rv_ratio:.2f}x" if iv_rv_ratio is not None else "N/A", help="How much more volatile the market expects the future to be compared to the recent past. >1 means a premium.")

if z_score is not None:
    st.metric("Log(IV/RV) Z-Score", f"{z_score:.2f}", help="Measures how expensive current IV is compared to its history relative to RV. A high Z-Score (>1) suggests IV is historically expensive and a potential selling opportunity.")
...
if strategy == "Sell Premium 🤑":
    with st.spinner(f"Finding best options to sell for ~{best_expiry_dte:.0f} DTE..."):
        ...
        expiry_df['ticker_data'] = expiry_df['instrument_name'].map(option_tickers); expiry_df.dropna(subset=['ticker_data'], inplace=True)
        expiry_df['delta'] = expiry_df['ticker_data'].apply(lambda x: x.get('delta')); expiry_df['price'] = expiry_df['ticker_data'].apply(lambda x: x.get('mark_price'))
    ...
    with act_col1:
        ...
        if best_put is not None:
            strategy_details['put'] = best_put.to_dict(); st.success(f"**Strike: ${best_put['strike']:.0f}**", icon="🎯"); st.write(f"Premium: `${best_put['price']:.2f}` | Delta: `{best_put['delta']:.3f}` | Premium/Spot: `{best_put['price']/live_eth_price:.2%}`")
        else: st.warning("No suitable Put found matching criteria.")
    with act_col2:
        st.markdown("**Candidate Call to Sell**")
        if best_call is not None:
            strategy_details['call'] = best_call.to_dict(); st.success(f"**Strike: ${best_call['strike']:.0f}**", icon="🎯"); st.write(f"Premium: `${best_call['price']:.2f}` | Delta: `{best_call['delta']:.3f}` | Premium/Spot: `{best_call['price']/live_eth_price:.2%}`")
        else: st.warning("No suitable Call found matching criteria.")
else:
...
aave_yield_monthly = (AAVE_APY / 12) * (ETH_DEPOSITED * ETH_PRICE_INITIAL); underlying_pnl = (eth_price_predicted - ETH_PRICE_INITIAL) * ETH_DEPOSITED; option_pnl = 0
if 'put' in strategy_details and strategy_details['put'] is not None: put_details = strategy_details['put']; option_pnl += put_details['price'] - max(0, put_details['strike'] - eth_price_predicted)
if 'call' in strategy_details and strategy_details['call'] is not None: call_details = strategy_details['call']; option_pnl += call_details['price'] - max(0, eth_price_predicted - call_details['strike'])
total_profit = underlying_pnl + aave_yield_monthly + option_pnl; collateral_initial = ETH_DEPOSITED * ETH_PRICE_INITIAL; apy = (total_profit / collateral_initial) * 12 * 100 if collateral_initial > 0 else 0
pnl_c1, pnl_c2, pnl_c3 = st.columns(3)
pnl_c1.metric("Projected Total Profit", f"${total_profit:,.2f}", f"{apy:.2f}% Projected APY"); pnl_c2.metric("AAVE Yield (1 Mo)", f"${aave_yield_monthly:,.2f}"); pnl_c3.metric("Net Option PnL", f"${option_pnl:,.2f}", help="PnL from sold options at your target price.")
