...
def determine_market_sentiment(iv, rv, vrp, vol_rank, rsi_daily, rsi_hourly, thresholds):
    ...
    if not is_vol_high_rank:
        ...
        if is_daily_bullish and is_hourly_confirming_bullish:
            sentiment = 'BULLISH'
            reason = f"Confirmed Uptrend (Daily RSI: {rsi_daily:.1f}) + High Vol Rank ({vol_rank:.0f}%) + Positive VRP ({vrp:.1%}). Ideal conditions to sell Puts."
        elif is_daily_bearish and is_hourly_confirming_bearish:
            sentiment = 'BEARISH'
            reason = f"Confirmed Downtrend (Daily RSI: {rsi_daily:.1f}) + High Vol Rank ({vol_rank:.0f}%) + Positive VRP ({vrp:.1%}). Ideal conditions to sell Calls for hedging."
        else:
            sentiment = 'NEUTRAL'
            reason = f"Conflicted/Neutral Trend (RSI Daily/Hourly: {rsi_daily:.1f}/{rsi_hourly:.1f}) + High Vol Rank ({vol_rank:.0f}%) + Positive VRP ({vrp:.1%}). Favorable for a Strangle."
...
def determine_perp_hedge_necessity(iv, rv, rsi_daily, daily_funding_rate, ltv, thresholds):
    ...
    if iv - rv > thresholds['min_vrp'] + 0.1: reasons.append(f"High VRP ({(iv-rv):.1%})")
...
with st.sidebar:
    ...
    st.header("6. Manual Overrides"); perp_hedge_override = st.selectbox("Perpetual Hedge Strategy", ["Automatic (Recommended)", "Force Short Hedge", "Force No Hedge"], help="Manually override the tactical perpetual hedge recommendation.")
